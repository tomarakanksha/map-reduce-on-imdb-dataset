...
public class MapperReducerMainCopy {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			for (int i=0;i<n;i++) {
				...
				if(reschk[i].contains("director.")){
					directorid=reschk[i].substring(9);
			...
			for(int k =0; k<j;k++) {
				System.out.println("directorid:"+directorid +"actorid:"+actorid[k]);
				if(directorid.equals(actorid[k])){
					result.set(res);
					context.write(key, result);
				}
			}
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		job.setJarByClass(MapperReducerMainCopy.class);
		...
		job2.setJarByClass(MapperReducerMainCopy.class);
		// job.setMapperClass(TokenizerMapper.class);
...
/*THIS IS GIVING right output w.r.t sql DB
 * 
 * 
 * 
 SELECT * FROM    imdb00.title_basics b 
INNER JOIN imdb00.TITLE_PRINCIPALS a ON  a.TCONST=b.TCONST   and a.category in ('actor','actress') and a.NCONST<>'\N'--and a.characters<> '\N'
INNER join imdb00.title_crew c ON a.TCONST=c.TCONST and c.directors<> '\N' 
and c.directors= a.NCONST --check this as dir are comma separated
